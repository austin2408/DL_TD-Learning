...
public:
	...
	// void init() { raw = 0; popup2(); popup2(); }
	// void init() { raw = 0; popup4(); popup4(); }
...
public:
	...
	state select_best_move(const board& b) const {
		...
		state* best = after;
		float value_sss_zegma = 0;
		for (state* move = after; move != after + 4; move++) {
			value_sss_zegma = 0;
			if (move->assign(b)) { // move up right left down , move is (s')
				board test = move->after_state();
		        board temp = test;
				std::vector<int> infro = temp.popup(2);

				for(int k=0; k<infro[0]; k +=1){
					board temp_2 = test;
					board temp_4 = test;
					temp_2.popup(2,1,k);
					temp_4.popup(4,1,k);
					value_sss_zegma = value_sss_zegma + 0.1*estimate(temp_4) + 0.9*estimate(temp_2);
				}
				float factor = (1.0/infro[0]);
				value_sss_zegma = factor*value_sss_zegma;
                move->set_value(move->reward() + value_sss_zegma);
	 ...
	 */
	void update_episode(std::vector<state>& path_s, std::vector<state>& path_ss, std::vector<state>& path_sss, float alpha = 0.1) const {
		float v_s = 0;
		path_s.pop_back();
		path_sss.pop_back();
		for (path_ss.pop_back()/* terminal state */; path_ss.size(); path_ss.pop_back()) {
			state& state_s = path_s.back();
			state& state_sss = path_sss.back();
			state& state_ss = path_ss.back();
			float td_error = state_ss.reward() + (v_s - estimate(state_s.after_state()));
			debug << "update error = " << td_error << " for state" << std::endl << state_s.after_state();
			float td_target = state_ss.reward() + estimate(state_sss.after_state());
			v_s = update(state_s.after_state(), alpha * td_error);
			path_s.pop_back();
			path_sss.pop_back();
		}
	}
...
int main(int argc, const char* argv[]) {
    ...
    tdl.add_feature(new pattern({ 0, 4, 5, 8, 9, 1 }));
    tdl.add_feature(new pattern({ 0, 4, 5, 1 }));
    tdl.add_feature(new pattern({ 2, 6, 10, 9, 8 }));
    tdl.add_feature(new pattern({ 2, 6, 5, 9, 8 }));
    tdl.add_feature(new pattern({ 9, 10, 6 }));
    tdl.add_feature(new pattern({ 1, 2, 6, 5 }));
    tdl.add_feature(new pattern({ 4, 5, 8, 9 }));
    tdl.add_feature(new pattern({ 0, 1, 4 }));
    tdl.add_feature(new pattern({ 12, 13, 14, 15 }));
    tdl.add_feature(new pattern({ 3, 7, 11, 15 }));
	tdl.add_feature(new pattern({ 2, 5, 8 }));
    tdl.add_feature(new pattern({ 7, 10, 13 }));
	tdl.add_feature(new pattern({ 3, 6, 9, 12 }));
	tdl.add_feature(new pattern({ 0, 5, 10, 15 }));
    tdl.add_feature(new pattern({ 0, 1, 2, 3 }));
	tdl.add_feature(new pattern({ 0, 4, 8, 12 }));
	tdl.add_feature(new pattern({ 4, 9, 14 }));
	tdl.add_feature(new pattern({ 1, 6, 11 }));
	tdl.add_feature(new pattern({ 5,6,10,9 }));


	// restore the model from file
	tdl.load("/home/austin/nctu_hw/DL/DL_hw2/weight/weight_0.1_100000_3");

	// train the model
	std::vector<state> path_s;
	std::vector<state> path_ss;
	std::vector<state> path_sss;
	path_s.reserve(20000);
	path_ss.reserve(20000);
	path_sss.reserve(20000);
	info << "start" <<std::endl;
	std::ofstream file;
	file.open("write_3.txt",std::ios::trunc);
	file.close();
	for (size_t n = 1; n <= total; n++) {
		...
		file.open("write_3.txt",std::ios::app);
		file << score<<std::endl;
		file.close();
		info<< "\rPlaying..."<<n<<" run"<<std::flush;

		// update by TD(0)
		tdl.update_episode(path_s, path_ss, path_sss, alpha);
		tdl.make_statistic(n, b, score);
		path_s.clear();
		path_ss.clear();
		path_sss.clear();
		if (n==25000){
            tdl.save("/home/austin/nctu_hw/DL/DL_hw2/weight_0.1_25000_3");
        }
        if (n==50000){
            tdl.save("/home/austin/nctu_hw/DL/DL_hw2/weight_0.1_50000_3");
        }
		if (n==75000){
            tdl.save("/home/austin/nctu_hw/DL/DL_hw2/weight_0.1_75000_3");
        }
	}

	// store the model into file
	tdl.save("/home/austin/nctu_hw/DL/DL_hw2/weight_0.1_100000_3");
